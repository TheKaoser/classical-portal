...
/** One chip is the work's form (or, on sonata and concerto pages, its instrument subtype). */
export function filterWorksByListedFilter<T extends { subtype: string | null }>(works: T[], slug: string): T[] {
  return filterWorksBySubtype(works, slug)
}
