    ...
    <ul className="divide-y divide-border">
      {works.map((work) => (
        <li key={work.id}>
          <Link
            href={`/works/${work.id}`}
            className="flex items-start gap-2 py-2.5 hover:bg-accent/40 -mx-2 px-2 rounded-md"
          >
            {isFlagged(work.popular) ? (
              <Star className="mt-0.5 h-3.5 w-3.5 shrink-0 fill-current text-foreground/70" />
            ) : (
              <span className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            )}
            <span className="min-w-0 flex-1">
              <span className="block text-sm text-foreground">{work.title}</span>
              {work.subtitle ? (
